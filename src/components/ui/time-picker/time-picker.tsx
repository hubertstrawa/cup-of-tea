...
export function TimePicker({ date, setDate, defaultPeriod, format24 }: TimePickerProps) {
  ...
  // Wrapper function to normalize dates before setting
  const handleSetDate = React.useCallback(
    (newDate: Date | undefined) => {
      if (newDate) {
        setDate(normalizeDateToMinutes(newDate));
      } else {
        setDate(newDate);
      }
    },
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {!format24 && (
        <div className="grid gap-1 text-center">
          <TimePeriodSelect period={period} setPeriod={setPeriod} date={date} setDate={handleSetDate} ref={periodRef} />
        </div>
      )}
    </div>
  );
